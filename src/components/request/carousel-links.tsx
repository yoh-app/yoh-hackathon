...
const CarouselLinks: React.FC<CategoriesProps> = ({ className = '', data }) => {
  ...
  return (
    <div className={className}>
          ...
          : data?.map((item: Record<string, any>) => (
            <SwiperSlide key={`item--key-${item.id}`}>
              ...
              >
                ...
                >
                  <Image
                    src={item.image?.original ?? placeholderImage}
                    layout={'fill'}
                    quality={100}
                    loader={({ src }) => {
                      return src;
      ...
      <div className="product-gallery-prev cursor-pointer absolute top-2/4 start-4 z-10 rounded-full bg-light shadow-xl border border-border-200 border-opacity-70 flex items-center justify-center text-heading transition-all duration-200 hover:bg-gray-100">
        <ChevronLeft className="w-6 h-6 p-1" />
      </div>
      <div className="product-gallery-next cursor-pointer absolute top-2/4 end-4 z-10 rounded-full bg-light shadow-xl border border-border-200 border-opacity-70 flex items-center justify-center text-heading transition-all duration-200 hover:bg-gray-100">
