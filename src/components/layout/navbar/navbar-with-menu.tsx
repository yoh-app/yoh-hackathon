...
import { useStripeEnabledQuery, useFindManyWebsiteQuery, usePermissionQuery } from '@generated';
...
const NavbarWithSearch: React.FC<Props> = ({ hideTypeMenu = false, page }) => {
  ...
  const { openSidebar, setSidebarView, isAuthorize, displayHeaderSearch, displayMobileSearch } = useUI();
  ...
  return (
    <>
      <Waypoint onLeave={showHeaderSearch} onEnter={hideHeaderSearch} onPositionChange={onWaypointPositionChange} />
      <header ref={navbarRef} className="site-header-with-search h-auto">
        ...
        >
          <ul className="flex flex-grow md:flex-grow-0">
            <li>
              ...
              >
                ...
                <NavbarIcon color={page?.navColor === 'white' && !displayHeaderSearch ? 'white' : 'black'} />
              </motion.button>
            </li>
            <li className="flex flex-grow align-items-center">
              <Logo displayHeaderSearch={displayHeaderSearch} page={page} className="mx-auto lg:mx-0" />
            </li>
          </ul>
          ...
          {/* <ul className="hidden pl-5 flex-grow lg:flex items-center flex-shrink-0 space-s-10">
            {menu?.map(({ href, icon, label }) => (
              <li key={`${href}${label}`}>
                <Link
                  href={href}
                  className="text-heading flex items-center transition duration-200 no-underline hover:text-accent focus:text-accent"
                >
                    ...
                    <li>
                      <Menu as="div" className="relative inline-block text-left">
                        <Menu.Button className={`flex items-center focus:outline-none`}>
                          <span className={`${page?.navColor === 'white' && !displayHeaderSearch ? 'text-white' : 'text-black'} flex items-center transition duration-200 no-underline hover:text-accent focus:text-accent`}>{label}</span>
                        </Menu.Button>
                        ...
                        >
                          <Menu.Items
                            as="ul"
                            className="absolute w-48 py-4 mt-1 origin-top-right bg-white rounded shadow-700 focus:outline-none"
                          >
                  ...
                  return (
                    <li key={`${pages[0].href}${pages[0].label}`}>
                      ...
                      <Link
                        href={pages[0].href}
                      >
                        <a className={`${query?.pageSlug === pages[0]?.slug && !!query?.pageSlug && !!pages[0]?.slug ? 'text-accent' : page?.navColor === 'white' && !displayHeaderSearch ? 'text-white' : 'text-black'
          ...
          <ul className="flex items-center flex-shrink-0 space-s-8">
            {/*isAuthorize ? (
              <li key="track-orders">
                <Link
                  href={ROUTES.ORDERS}
            ...
            <div className='md:mr-24'><ConnectButton /></div>
            {address && <Link href='/nft'><a className={page?.navColor === 'white' && !displayHeaderSearch ? 'text-white' : 'text-black'}>NFT</a></Link>}
            {isAuthorized && (website?.paymentMethod === 'crypto' ? website?.walletAddress : stripeEnabledData?.stripeEnabled) ? (
              <>
                {pathname !== '/products/[slug]' && (
                  <li>
                    <RequestButton hasRequests={hasRequests} displayHeaderSearch={displayHeaderSearch} page={page} />
                  </li>
                )}
                {/* <li>
                  <CartButton />
            ...
            {innerWidth >= 959 ? (
              <li>
                <SearchButton hasRequests={hasRequests} displayHeaderSearch={displayHeaderSearch} page={page} />
              </li>
            ) : (
              []
            )}
