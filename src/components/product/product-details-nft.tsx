...
import { usePayProductMutation, usePreparePurchaseMutation, useUpdateOrderMutation } from '@generated';
...
const ProductDetails: React.FC<Props> = ({ view, setView, product, productData, orderedProduct, nftBought }) => {
  const {
    name,
  ...
  const [payProduct] = usePayProductMutation();
  ...
  const { config: polygonConfig } = usePrepareContractWrite({
    addressOrName: product?.editionAddress,
    contractInterface: PolygonSingleEditionMintableAbi.abi,
  ...
  useEffect(() => {
    ...
    if (order?.id && processingTransaction) {
  ...
  }, [order, klaytnWrite, polygonWrite, product]);
  ...
  return (
    <article className="rounded-lg bg-light">
      ...
      <div className="flex flex-col md:flex-row">
        <div className="md:w-1/2 md:max-w-[600px] p-6 lg:px-8">
          <div className="product-gallery h-full relative">
            {discount && (
              <div
                className="rounded-full text-xs leading-6 font-semibold px-3 bg-accent-300 font-accent-900 text-light absolute top-4 end-4 z-10"
        ...
        <div className="flex flex-col flex-grow items-start p-6 lg:px-8">
          <div className="w-full">
            <h1 className="font-semibold text-lg md:text-xl xl:text-2xl tracking-tight text-heading text-[#212B36]">
              {name}
            </h1>
            <div className="mt-3 md:mt-4 text-body text-sm text-[#637381] leading-7">
              {t('edition-address')}:&nbsp;{editionAddress}
            </div>
            ...
            <div className="my-5 md:my-10 flex items-center">
              {!isEmpty(variations) ? (
                ...
                <span className="flex items-center">
                  ...
                  {discount && (
                    <del className="text-sm md:text-base font-normal text-muted text-[#637381] ms-2">
                      <span className="uppercase">{currency}</span> {price.replace('$', '')}
                    </del>
                  )} */}
                  {!product?.isExternalNft && <div className="text-sm md:text-base font-normal text-muted text-[#637381] ms-2">
            ...
            {product?.isExternalNft ? <div className="mt-4 md:mt-6 flex flex-col lg:flex-row items-center">
              <div className="mb-3 lg:mb-0 w-full">
                ...
                ) : (<Link href={product?.externalUrl ? product.externalUrl : '#'}><button
                  disabled={product?.externalUrl ? false : true}
                  className={product?.externalUrl ? "py-4 px-5 w-full flex items-center justify-center text-sm lg:text-base font-light rounded text-light bg-accent hover:bg-accent-hover transition-colors duration-300 focus:outline-none focus:bg-accent-hover" : "py-4 px-5 w-full flex items-center justify-center text-sm lg:text-base font-light rounded text-light bg-accent hover:bg-accent-hover transition-colors duration-300 focus:outline-none focus:bg-accent-hover border !bg-gray-300 hover:!bg-gray-300 border-border-400 !text-body cursor-not-allowed"}
                >
