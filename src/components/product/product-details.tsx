...
const ProductDetails: React.FC<Props> = ({ view, setView, product, orderedProduct, productData, nftBought }) => {
  let {
    name,
  ...
  const { themeColor, currency } = useSettings();
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="flex flex-col flex-grow items-start p-6 lg:px-8">
          <div className="w-full">
            <h1 className="font-semibold text-lg md:text-xl xl:text-2xl tracking-tight text-heading text-[#212B36]">{name}</h1>
            ...
            <div className="my-5 md:my-10 flex items-center">
              {!isEmpty(variations) ? (
                ...
                <span className="flex items-center">
                  ...
                  {discount && <del className="text-sm md:text-base font-normal text-muted text-[#637381] ms-2"><span className="uppercase">{currency}</span> {price.replace("$", "")}</del>}
                </span>
              )}
            </div>
                  ...
                  orderedProduct ? (
                    ...
                    <button
                      onClick={async () => {
                        ...
                        window.location.replace(result.data.prepareStripePurchase.session.url)
                      }}
                    ...
                    >
                      <span className="flex-1">{t('text-purchase')}</span>
                    </button>
                  )
                ) : (
                  []
            ...
            {orderedProduct && <>{orderedProduct?.minted ? <div className="mt-4 md:mt-6 flex flex-col lg:flex-row items-center"><div className="mb-3 lg:mb-0 w-full">
              <button
                disabled
                className="py-4 px-5 w-full flex items-center justify-center text-sm lg:text-base font-light rounded text-light bg-accent hover:bg-accent-hover transition-colors duration-300 focus:outline-none focus:bg-accent-hover border !bg-gray-300 hover:!bg-gray-300 border-border-400 !text-body cursor-not-allowed"
              >
                <span className="flex-1">{t('text-mint')}</span>
              </button>

            </div></div> : <div className="mt-4 md:mt-6 flex flex-col lg:flex-row items-center"><div className="mb-3 lg:mb-0 w-full">
              {address ? <button
                disabled={disabled}
                onClick={async () => {
