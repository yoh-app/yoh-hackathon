...
export default function SearchPage({ website }) {
  ...
  return (
    <div className="min-h-[100vh] bg-[#F8F8F8]">
      <section className={`max-w-1920 w-full mx-auto pt-[100px] ${isSearched ? 'onSearching' : ''}`}>
        ...
        <SearchResultStyle className={cn('opacity-0 px-4 mt-[24px]', { isSearched: isSearched })}>
          ...
          <Grid container sx={{ marginTop: '24px', padding: '16px 0' }} spacing={{ xs: 2, md: 4 }}>
                ...
                return (
                  <Grid component={'a'} href={`/products/${item?.slug}`} key={item.id} item xs={12} md={6} lg={4} xl={3}>
                    <SearchCardStyle sx={{ height: '115px', padding: '12px', borderRadius: '14px', display: 'flex' }}>
                      <div
                        className="shrink-0 w-[90px] h-[90px] rounded-[14px]"
                        style={{
            ...
            {/* {tabIndex === 0 || tabIndex === 2
              ? linksData?.findManyLink.map((item) => {
                  return (
                    <Grid key={item.id} item xs={12} md={6} lg={4} xl={3}>
                      ...
                      >
                        ...
                        ></div>
                        <div className="grow self-center px-6">{item.name}</div>
                        <SearchChipStyle className="chip-links" size="small" label={t('search-chip-link')} />
                      </SearchCardStyle>
                    </Grid>
                  );
                })
            ...
            {(productsData?.findManyProduct.length || 0) +
              ...
              0 ? (
              <Grid item xs={12}>
                <div className="h-[115px] pt-[12px] text-[#B8B8B8] flex items-center justify-center">
                  {t('search-empty')}
                </div>
              </Grid>
            ) : (
